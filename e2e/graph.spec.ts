...
test('Reset layout clears the saved manual positions', async ({ page }) => {
  ...
  // (D77-DEF-1). An absent row serialises to "null", which differs from the seeded layout, so the
  // original "absent OR different" intent is preserved exactly.
  await expect
    .poll(
      () =>
        page.evaluate(async () => {
    ...
    .not.toEqual(JSON.stringify(manual));
});
  ...
  await expect
    .poll(
      () =>
        page.evaluate(async () => {
          const row = await window.__rb!.db.meta.get('graphPositions');
          return Object.keys((row?.value as Record<string, unknown> | undefined) ?? {}).length;
        }),
      { timeout: 15_000 },
    )
    .toBeGreaterThan(0);
